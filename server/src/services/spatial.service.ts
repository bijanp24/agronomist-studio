...
export function calculateTerrainFlow(grid: ElevationGrid): TerrainFlowResult {
  ...
  return {
    points,
    minSlopePercent: slopes.length ? round4(Math.min(...slopes)) : 0,
    maxSlopePercent: slopes.length ? round4(Math.max(...slopes)) : 0,
    avgSlopePercent: slopes.length ? round4(slopes.reduce((s, v) => s + v, 0) / slopes.length) : 0,
